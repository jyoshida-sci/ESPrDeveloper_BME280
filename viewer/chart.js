(function(global) {
  ...
  function get_graph_data_hum(data) {
    return data.map(function(d) {
      return {
        date: new Date(d.timestamp),
        value: d.humi
  ...
  function get_graph_data_temp(data) {
    return data.map(function(d) {
      return {
        date: new Date(d.timestamp),
        value: d.temp
  ...
  createChart.prototype.initialDraw = function() {
    ...
    this.xScale.domain(d3.extent(dataset, function(d) {
      return d.date;
    }));
    this.yScale.domain(d3.extent(dataset, function(d) {
      return d.value;
    }));
